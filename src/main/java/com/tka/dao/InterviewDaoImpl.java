...
public class InterviewDaoImpl {
	...
	public String updateInterview(Interview interview, long id) {
		Transaction transaction = null;
		try (Session session = factory.openSession()) {
			transaction = session.beginTransaction();
			Interview inter=session.get(Interview.class, id);  // Update the interview object
			inter.setStudentId(interview.getStudentId());
			inter.setJobApplicationId(interview.getJobApplicationId());
			inter.setInterviewDate(interview.getInterviewDate());
